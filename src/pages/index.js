  ...
  useEffect(() => {
    if (histogramData && chartRef.current) {
      ...
      const intervals = [
        ...
        { min: 10000000, max: Infinity, label: 'Más de $9.999.999' },
      ];

      const data = intervals.map(interval => {
        const filteredData = histogramData.filter(data => {
          return data.money >= interval.min && data.money <= interval.max;
        });

        return filteredData.reduce((sum, data) => sum + data.money, 0);
      });
